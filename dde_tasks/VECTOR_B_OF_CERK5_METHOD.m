function answer =  VECTOR_B_OF_CERK5_METHOD( theta, stage )
    % theta     - ПАРАМЕТР ВРЕМЕНИ, В КОТОРОМ ВЫЧИСЛЯЕМ ИНТЕРПОЛИРОВАННОЕ
    %             ЗНАЧЕНИЕ ВЕКТОРА МЕТОДА CERK ПЯТОГО ПОРЯДКА
    % stage     - ТЕКУЩИЙ ЭТАП МЕТОДА
    
    % ВЕКТОР КОЭФФИЦИЕТОВ c МЕТОДА CERK ПЯТОГО ПОРЯДКА
    % ЗНАЧЕНИЕ КАЖДОГО ЭЛЕМЕНТА вектора ЗАВИСИТ ОТ theta
    switch stage
        case 1
